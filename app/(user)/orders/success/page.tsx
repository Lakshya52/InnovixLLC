      ...
      {loading ? (
        ...
        <>
          <div className="flex items-center justify-center relative">
            ...
            <div className="w-30 h-30 bg-(--accent) rounded-full animate-ping opacity-20 absolute" />
          </div>
          ...
          {assignedKey && (
            <div className="bg-(--text-main)/[0.03] border border-(--accent)/30 p-8 rounded-[30px] w-full max-w-xl mb-12 backdrop-blur-sm relative overflow-hidden group">
              ...
              <div className="flex items-center gap-3 mb-4 justify-center">
                ...
                <span className="text-xs font-bold uppercase tracking-widest text-(--accent)">Your Digital Key</span>
              </div>
              <p className="text-3xl font-mono font-bold text-(--text-main) tracking-wider break-all">
                {assignedKey}
              </p>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-6">
            <Link
              href="/keys"
              className="bg-(--accent) hover:bg-(--accent) text-(--bg-dark) font-bold py-4 px-10 rounded-2xl transition-all duration-300 shadow-[0_0_30px_rgba(110,221,134,0.4)] flex items-center gap-2 group"
            >
              View My Vault
            ...
            <Link
              href="/products"
              className="bg-(--text-main)/[0.05] hover:bg-(--text-main)/[0.1] text-(--text-main) font-bold py-4 px-10 rounded-2xl transition-all duration-300 border border-(--text-main)/10"
            >
              Continue Shopping
            </Link>
          </div>
  ...
  return (
    <Suspense fallback={
      <div className="min-h-[80vh] flex flex-col items-center justify-center gap-6">
        <Loader2 className="w-16 h-16 text-(--accent) animate-spin" />
        <h2 className="text-2xl font-bold font-grotesk text-(--text-main)">Loading Order Data...</h2>
      </div>
    }>
      <OrderSuccessContent />
    </Suspense>
