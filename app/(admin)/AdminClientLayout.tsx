...
export default function AdminClientLayout({
  ...
  return (
    <div className="flex min-h-screen w-full bg-(--bg-less-dark) text-(--text-main) font-inter">
      {/* Sidebar */}
      <aside
        className={`bg-(--bg-dark) border-r border-(--bg-dark) flex flex-col p-6 fixed top-0 bottom-0 left-0 z-50 transition-all duration-300 ${
          sidebar ? "w-[260px]" : "w-[85px]"
        }`}
      >
        <div
          className={`text-lg font-bold text-(--accent) mb-10 flex items-center ${
            sidebar ? "justify-between gap-2" : "justify-center"
          }`}
        >
          <span
            className={`${
              sidebar ? "block" : "hidden"
            } transition-all duration-300 (--text-main)space-nowrap`}
          >
            Admin Dashboard
          </span>
          <button
        ...
        <nav className="flex flex-col gap-2 flex-grow overflow-hidden">
          {navItems.map((item) => (
            ...
            >
              ...
              <span
                className={`font-medium text-sm (--text-main)space-nowrap overflow-hidden transition-all duration-300 ${
                  sidebar ? "w-auto opacity-100 ml-0" : "w-0 opacity-0 ml-[-12px]"
                }`}
              >
                {item.name}
              </span>
        ...
        <div className="mt-auto pt-6 border-t border-(--bg-dark)">
          <form action={logout}>
            <button
              className={`flex items-center gap-3 px-4 py-3 rounded-lg text-(--text-main) hover:bg-red-500/10 hover:text-red-400 transition-all duration-200 w-full text-left cursor-pointer ${
                sidebar ? "justify-start" : "justify-center px-0"
              }`}
            >
              ...
              <span
                className={`font-medium text-sm (--text-main)space-nowrap overflow-hidden transition-all duration-300 ${
                  sidebar ? "w-auto opacity-100" : "w-0 opacity-0"
                }`}
              >
                Logout
              </span>
      ...
      <div
        className={`flex-grow flex flex-col transition-all duration-300 ${
          sidebar ? "ml-[260px]" : "ml-[85px]"
        }`}
      >
        {/* Topbar */}
        <header className="h-[80px] px-8 flex items-center justify-between border-b border-(--bg-dark) bg-(--bg-dark) sticky top-0 z-40">
          <div className="flex items-center bg-(--bg-dark) border border-(--bg-less-dark) rounded-full px-4 py-2 w-[400px] group focus-within:border-(--accent)/50 transition-all mr-10 max-w-[500px]">
            <Search
              size={18}
              className="text-gray-500 group-focus-within:text-(--accent) transition-colors"
            />
            <input
              type="text"
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="w-[35px] h-[35px] rounded-full bg-(--bg-dark) flex items-center justify-center border border-(--bg-less-dark) overflow-hidden cursor-pointer hover:border-(--accent)/50 transition-all">
                <img
                  src={
                    user?.image ||
                    `https://api.dicebear.com/7.x/avataaars/svg?seed=${user?.id || "Admin"}`
                  }
