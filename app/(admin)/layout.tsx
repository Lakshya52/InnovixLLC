...
export default function AdminLayout({
  ...
  return (
    <div className="flex min-h-screen w-full bg-[#0d0d0d] text-[#e2e2e2] font-inter">
      {/* Sidebar */}
      <aside className={`bg-[#121212] border-r border-[#1f1f1f] flex flex-col p-6 fixed top-0 bottom-0 left-0 z-50 transition-all duration-300 ${sidebar ? "w-[260px]" : "w-[85px]"}`}>
        <div className={`text-lg font-bold text-[#6eDD86] mb-10 flex items-center ${sidebar ? 'justify-between gap-2' : 'justify-center'}`}>
          <span className={`${sidebar ? "block" : "hidden"} transition-all duration-300 whitespace-nowrap`} >
            Admin Dashboard
          </span>
          <button 
      ...
      <div className={`flex-grow flex flex-col transition-all duration-300 ${sidebar ? "ml-[260px]" : "ml-[85px]"}`}>
        {/* Topbar */}
        <header className="h-[80px] px-8 flex items-center justify-between border-b border-[#1f1f1f] bg-[#0d0d0d] sticky top-0 z-40">
          <div className="flex items-center bg-[#1a1a1a] border border-[#2a2a2a] rounded-full px-4 py-2 w-[400px] group focus-within:border-[#6eDD86]/50 transition-all w-full mr-10 max-w-[500px]">
          ...
          <div className="flex items-center gap-6">
            <div className="relative cursor-pointer group">
              <Bell size={20} className="text-[#a0a0a0] group-hover:text-red-500 transition-colors" />
              <span className="absolute -top-1 -right-1 w-2 h-2 bg-red-500 rounded-full border-2 border-[#0d0d0d]"></span>
            </div>
            
            <div className="flex items-center gap-3">
               <div className="text-right hidden sm:block">
                  <p className="text-xs font-bold text-[#e2e2e2]">Full Name</p>
