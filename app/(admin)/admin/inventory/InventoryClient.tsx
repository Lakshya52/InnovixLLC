  ...
  return (
    <div className="p-8 mx-auto w-full max-w-[1400px]">
      ...
      <div className="mb-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-4xl font-bold mb-2">Product <span className="text-(--accent)">Inventory</span></h1>
        ...
        <div className="flex items-center gap-3">
          ...
          <Link
            ...
            className="flex items-center gap-2 bg-(--accent) text-(--bg-dark) px-6 py-3 rounded-2xl font-bold text-sm hover:bg-(--accent) transition-all cursor-pointer shadow-[0_0_20px_rgba(110,221,134,0.2)]"
          >
            <Plus size={18} />
            Add New Product
          </Link>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
        <div className="bg-(--bg-dark) border border-(--bg-dark) rounded-[32px] p-8 relative overflow-hidden group border-l-4 border-l-(--accent)">
          <div className="flex justify-between items-start mb-6">
            ...
            <div className="p-3 bg-(--bg-dark) rounded-2xl text-(--accent) shadow-[0_0_15px_rgba(110,221,134,0.1)]">
              <Package size={24} />
            </div>
          </div>
        ...
        <div className="bg-(--bg-dark) border border-(--bg-dark) rounded-[32px] p-8 relative overflow-hidden group">
          <div className="flex justify-between items-start mb-6">
            ...
            <div className="p-3 bg-(--bg-dark) rounded-2xl text-yellow-500">
              <Eye size={24} />
            </div>
          </div>
        ...
        <div className="bg-(--bg-dark) border border-(--bg-dark) rounded-[32px] p-8 relative overflow-hidden group border-l-4 border-l-red-500/50">
          <div className="flex justify-between items-start mb-6">
            <div>
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1">Low Stock Alerts</p>
              <h3 className="text-4xl font-bold text-(--text-main)">{String(stats.lowStock).padStart(2, '0')}</h3>
            </div>
            <div className="p-3 bg-(--bg-dark) rounded-2xl text-red-500">
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-(--bg-dark)">
              {filteredProducts.map((p) => {
                ...
                return (
                  <tr key={p.id} className="group hover:bg-(--bg-dark)/30 transition-all">
                    ...
                    <td className="px-6 py-8">
                      <span className="px-3 py-1 bg-(--bg-dark) border border-(--bg-less-dark) rounded-lg text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                        {p.category}
                      </span>
                    </td>
                    <td className="px-6 py-8">
                      <p className="text-xl font-bold text-(--accent)">${p.price.toFixed(2)}</p>
                    </td>
                    <td className="px-6 py-8">
                      <div className="max-w-[140px]">
                        <div className="flex justify-between items-end mb-2">
                          <p className="text-[10px] font-bold text-gray-400">
                            {stockCount} keys <span className="text-(--accent)/60 ml-1">{stockCount > 50 ? '85%' : stockCount > 0 ? 'Low' : 'Empty'}</span>
                          </p>
                        </div>
                        <div className="h-1.5 w-full bg-(--bg-dark) rounded-full overflow-hidden">
                          <div
                            className={`h-full transition-all duration-500 shadow-[0_0_8px] ${stockCount > 50 ? 'bg-(--accent) shadow-(--accent)/20' :
                    ...
                    <td className="px-6 py-8">
                      <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full border text-[10px] font-bold uppercase tracking-widest ${p.status === 'Live' ? 'border-(--accent)/20 bg-(--accent)/10 text-(--accent)' : 'border-gray-500/20 bg-gray-500/5 text-gray-500'
                        }`}>
                        <div className={`w-1.5 h-1.5 rounded-full ${p.status === 'Live' ? 'bg-(--accent) shadow-[0_0_6px_#6eDD86]' : 'bg-gray-600'}`}></div>
                        {p.status}
                      </div>
                    </td>
      ...
      {/* System Log Section */}
      <div className="bg-(--bg-dark) border border-(--bg-dark) rounded-[40px] p-10">
        ...
        <div className="space-y-8 mb-10">
          {initialLogs.map((log) => (
            <div key={log.id} className="relative pl-8 before:absolute before:left-0 before:top-1.5 before:w-2.5 before:h-2.5 before:rounded-full before:bg-(--accent) before:shadow-[0_0_10px_rgba(110,221,134,0.3)]">
              <div className="flex items-center justify-between mb-1">
                <p className="text-sm font-bold text-(--text-main)">{log.message}</p>
        ...
        <button className="w-full bg-(--accent) text-(--bg-dark) py-4 rounded-2xl font-bold text-xs uppercase tracking-widest hover:bg-(--accent) transition-colors cursor-pointer active:scale-[0.995]">
          View Full Log
        </button>
      </div>
