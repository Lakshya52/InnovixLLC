      ...
      <div className="max-w-[1400px]  mx-auto grid grid-cols-1 lg:grid-cols-2 gap-20 items-start">
        ...
        <div className="flex flex-col items-start justify-between gap-12">
          <div className="space-y-6">
            <h1 className="text-6xl font-bold font-grotesk leading-[0.9] tracking-tighter">
              How can we <span className="text-(--accent)">help</span> <br />
              <span className="text-(--accent)">you</span> today?
            </h1>
            <p className="text-gray-400 text-xl font-inter max-w-lg leading-relaxed">
          ...
          <div className="flex w-full gap-6">
            {/* Blogs Card */}
            <Link href="/blogs" className="group w-full relative overflow-hidden bg-white/[0.03] border border-white/5 rounded-[40px] p-10 hover:bg-white/[0.06] transition-all duration-500">
        ...
        <div className="relative group lg:sticky lg:top-32">
          {/* Glass Card */}
          <div className="bg-white/[0.03] border border-white/5 rounded-[50px] overflow-hidden flex flex-col h-[80dvh] shadow-2xl shadow-black/40">

            {/* Chat Header */}
            <div className="bg-white/[0.05] border-b border-white/5 p-8 flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className="relative">
                  <img src="https://api.dicebear.com/9.x/glass/svg?seed=Sara" alt="Sara" className="w-14 h-14 rounded-2xl object-cover bg-white/10" />
                  <div className="absolute -right-1 -bottom-1 w-4 h-4 rounded-full bg-(--accent) border-4 border-(--bg-dark)" />
                </div>
                <div>
                  <h3 className="text-xl font-bold font-grotesk">Sara Sterling</h3>
              ...
              <div className="flex items-center gap-3">
                ...
                <Link href="/" className="w-10 h-10 rounded-full flex items-center justify-center text-gray-500 hover:text-white hover:bg-white/5 transition-all">
                  <X size={20} />
                </Link>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-10 space-y-10 custom-scrollbar">
              <div className="flex justify-center">
                <span className="bg-white/[0.03] border border-white/5 px-6 py-2 rounded-full text-[10px] font-bold text-gray-500 tracking-widest uppercase">
              ...
              {messages.map((msg: any) => (
                <div key={msg.id} className={`flex gap-6 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                  ...
                  <div className={`flex flex-col gap-3 max-w-[80%] ${msg.role === 'user' ? 'items-end' : ''}`}>
                    <div className={`p-6 rounded-[30px] font-inter leading-relaxed text-sm ${msg.role === 'user'
                      ? 'bg-(--accent) text-black shadow-[rgba(110,221,134,0.3)_0px_20px_50px]'
                      : 'bg-white/[0.04] text-gray-300 border border-white/5'
                      }`}>
                      {msg.text}
                      {msg.showRegister && (
                        <div className="mt-4 pt-4 border-t border-white/10">
                          <Link href="/registration" className="inline-flex items-center gap-2 bg-(--accent) text-black px-4 py-2 rounded-xl text-xs font-bold hover:scale-105 transition-all">
            ...
            {/* Chat Input */}
            <div className="p-8 pb-12 bg-gradient-to-t from-white/5 to-transparent border-t border-white/5">
              <div className="relative bg-white/[0.04] border border-white/10 rounded-[30px] p-2 flex items-center gap-2 focus-within:border-(--accent)/30 transition-all">
                <input
                  type="text"
                  ...
                  className="flex-1 bg-transparent px-6 py-4 text-sm font-inter outline-none"
                />
                <div className="flex items-center gap-1 pr-2">
                  <button onClick={handleSend} className="w-12 h-12 rounded-2xl bg-(--accent) text-black flex items-center justify-center hover:bg-[#5dbb72] transition-all shadow-lg shadow-(--accent)/20 ml-2">
                    <Send size={20} />
                  </button>
                </div>
              </div>
